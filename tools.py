...
ChunkStrategy: TypeAlias = Literal["legacy", "greedy"]
...
def _legacy_group_children(
    node: Node, children: list[Node], chunk_size: int
) -> list[list[Node]]:
    ...
    groups_lengths = [
        len(group)
        for group in mitertools.split_when(
            cumulative_sizes, lambda x, y: x // chunk_size != y // chunk_size
        )
    ]
...
def _split_in_place(
    *,
    ...
    original_files = set(str(n.data.path) for n in leaf_nodes)
    ...
    if oversized := [n for n in leaf_nodes if (n.data.size or 0) > chunk_size]:
        ...
        log.warning(
            ...
            "Largest: "
            + ", ".join(
                f"{n.data.path} ({_bytes_to_str(n.data.size or 0)})"
                for n in largest[:3]
            )
        )
    ...
    *,
    ...
    if strategy not in _CHUNK_STRATEGIES:
        raise ValueError(
            ...
            f"{', '.join(_CHUNK_STRATEGIES)}."
        )
    return _CHUNK_STRATEGIES[strategy](
        tree=tree, chunk_size=chunk_size, min_zip_depth=min_zip_depth
    )
